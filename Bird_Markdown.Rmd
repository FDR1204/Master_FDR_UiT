...
setwd("C:/Users/femke/OneDrive/Dokumente/UIT/RMASTER/Bird_Data/Fugler2024")
...
Now I will combine all Dataframes and look for Statistical differences between groups of birds.
```{r}
...
ggplot(data = relevant_groups, 
  ...
  geom_bar(position = "fill") +
  theme_bw() +
  labs(x = "Location", y = "Percentage (%)", fill = "Avifauna") +
...
CQ<-chisq.test(Bird_Data$Site, Bird_Data$Bird, correct=FALSE)
...
#CQT<-chisq.posthoc.test(phtable, method = "bonferroni", round = 6)
#summary(CQT)
#row_wise_fisher_test(phtable, p.adjust.method = "holm", detailed = FALSE, ...)
#posthoc
...
```
tabyl(relevant_groups, Bird, Site) %>%
  adorn_percentages("col") %>%
  adorn_pct_formatting(digits = 1)

phtable<-table(relevant_groups$Site, relevant_groups$Bird)
View(phtable)
